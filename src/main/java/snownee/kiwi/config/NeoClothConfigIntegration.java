...
public class NeoClothConfigIntegration {
	public static void init() {
		...
		IConfigScreenFactory factory = (container, screen) -> Objects.requireNonNull(ClothConfigIntegration.create(
				screen,
				container.getModId()));
		for (String mod : mods) {
			...
			ModList.get().getModContainerById(mod).ifPresent($ -> $.registerExtensionPoint(IConfigScreenFactory.class, factory));
		}
	}
}
