...
public class BlockPredicateHelper {
	...
	public static final BlockPredicate ANY = new BlockPredicate(Optional.empty(), Optional.empty(), Optional.empty(), DataComponentMatchers.ANY);
	...
	public static boolean fastMatch(BlockPredicate predicate, BlockState blockstate/*, Supplier<BlockEntity> beGetter*/) {
		...
		if (predicate.blocks().isPresent() && !predicate.blocks().get().contains(blockstate.getBlock().builtInRegistryHolder())) {
			return false;
		}
